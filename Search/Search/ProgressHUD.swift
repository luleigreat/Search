...
class HYBProgressHUD: UIView {
    ...
    class func show(status: String) {
        sharedInstance().configureHUD(status, image: nil, isSpin: false, isHide: true)
    }
    ...
    private func setStyle(style: HYBProgressHUDStyle) {
        ...
        case .WhiteHUDStyle:
            ...
            successImage = UIImage(named: "success")
            errorImage = UIImage(named: "ProgressHUD.bundle/error-white.png")
            break
        }
    }
    ...
    func hideWhenTimeout() {
        autoreleasepool { () -> () in
            ...
            let sleepTime:NSTimeInterval = NSTimeInterval(2.0)
